...
export async function codeGrantAuth(redirectUrl) {
  try {
    ...
    await axios.get(process.env.REACT_APP_API_BASE_URL + '/code_grant_auth');
  } catch (error) {
    handleError(error);
  }
}
...
export async function getStatus(setStatus, setAuthType) {
  try {
    let response = await axios.get(process.env.REACT_APP_API_BASE_URL + '/get_status', {
      withCredentials: true,
    });
...
export async function jwtAuth(navigate) {
  try {
    await axios.get(process.env.REACT_APP_API_BASE_URL + '/jwt_auth', {
      withCredentials: true,
    });
  } catch (error) {
    navigate('/');
...
export async function checkPayment(setShowJWTModal, navigate) {
  try {
    const response = await axios.get(process.env.REACT_APP_API_BASE_URL + '/check_payment', {
      withCredentials: true,
    });
    return response;
  } catch (error) {
