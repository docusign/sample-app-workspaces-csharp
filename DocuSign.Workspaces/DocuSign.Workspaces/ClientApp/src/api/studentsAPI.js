...
export async function getStatus(fromDate) {
  try {
    const response = await axios.get(process.env.REACT_APP_API_BASE_URL + '/requests', {
      params: {
        'from-date': fromDate,
      },
      withCredentials: true,
...
export async function getStatusDocument(envelopId, documentId) {
  try {
    const response = await axios.get(process.env.REACT_APP_API_BASE_URL + '/requests/download', {
      params: {
